...
public class InMemoryTaskRepository implements TaskRepository {
   ...
   private final AtomicLong activityLogIdCounter = new AtomicLong(0);
   ...
   private void createSeedTask(Long refId, ReferenceType refType, Task task, Long assigneeId, TaskStatus status, Priority priority) {
       ...
       activityLog.setId(activityLogIdCounter.incrementAndGet());
       activityLog.setTaskId(newId);
       activityLog.setDescription("Task created with " + status + " status");
       activityLog.setUserId(0L); // System
       activityLog.setUsername("System");
       activityLog.setTimestamp(Instant.now());
       newTask.getActivityHistory().add(activityLog);
   ...
   public ActivityLog addActivityLog(Long taskId, String description, Long userId, String username) {
       TaskManagement task = findById(taskId).orElseThrow(() -> 
       ...
       task.getActivityHistory().add(activityLog);
       save(task);
       
       return activityLog;
   }
   
   public Comment addComment(Long taskId, String text, Long userId, String username) {
       TaskManagement task = findById(taskId).orElseThrow(() -> 
           new IllegalArgumentException("Task not found with id: " + taskId));
